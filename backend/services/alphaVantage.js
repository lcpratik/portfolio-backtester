import axios from "axios";
 ...
 export async function getDailyPrices(ticker) {
    try {
        ...
        // positive value means b comes before a, and 0 means they are equal. So, if we get negative value that means it is already sorted in ascending order. 
        // If we get positive value that means it is sorted in descending order. If we get 0 that means they are equal and we can keep them in the same order.
        return prices;
    } catch (error) {
        console.error(error);
        throw error;    
